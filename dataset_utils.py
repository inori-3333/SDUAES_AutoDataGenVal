...
def merge_dataset(dataset_list, merged_dataset_dir):
    ...
    # 用于存储合并后的数据集
    merged_data = {"code": [], "description": []}
    
    # 固定的键名列表
    keys = ["block_summary", "detailed_global_summary", "high_level_global_summary"]

    # 假设 dataset_list 中每个元素对应不同的文件内容，但代码是相同的
    base_dataset = load_from_disk(dataset_list[0])  # 加载第一个数据集作为基础
    for i in range(len(base_dataset["code"])):
        merged_description = {}
        code = base_dataset["code"][i]
        
        # 遍历每个数据集的 description 字段，并将它们合并
        for idx, dataset_dir in enumerate(dataset_list):
            dataset = load_from_disk(dataset_dir)
            # 使用固定的键名来存储 description
            key = keys[idx]
            merged_description[key] = dataset["description"][i]
        
        # 将合并后的 description (作为 dict) 和相应的 code 存入新的数据集中
        merged_data["code"].append(code)
        merged_data["description"].append(merged_description)  # 合并 description 为 dict

    # 将合并后的数据集转换为 Dataset 对象并打乱顺序
    new_merged_dataset = Dataset.from_dict(merged_data)
    new_merged_dataset = new_merged_dataset.shuffle()
...
class Data4AIGChipDataset:
    ...
    def load_code_entries(self, documented_src = "documented_code"):
        ...
        for doc_dir in self.dataset_metadata_dir:
            ...
            print(individual_metadata_file)
...
if __name__ == "__main__":
    ...
    if package_merged_dataset:
        dataset_dirs = [
                        "/home/mlserver/mg-verilog/auto_data_gen_val/preprocess_data/process_data/test_output/packaged_dataset/llm2_block_summary_to_pure_code_one_shot_dataset",
                        "/home/mlserver/mg-verilog/auto_data_gen_val/preprocess_data/process_data/test_output/packaged_dataset/detailed_description_dataset",
                        "/home/mlserver/mg-verilog/auto_data_gen_val/preprocess_data/process_data/test_output/packaged_dataset/simple_description_dataset"
                        ]
